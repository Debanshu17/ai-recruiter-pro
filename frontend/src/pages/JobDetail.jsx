  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <h1>{job.title}</h1>
        <Link to={`/compare?job_id=${id}`} className="btn btn-secondary">Compare Candidates</Link>
      </div>

      <div className="grid grid-cols-3">
        <div className="card" style={{gridColumn: 'span 2'}}>
          <h3>Bulk Resume Upload</h3>
          <p className="mb-4 text-sm" style={{color: 'var(--text-secondary)'}}>Upload multiple PDF resumes to screen candidates against this job description.</p>
          
          <div className="upload-zone" onClick={() => fileInputRef.current?.click()}>
            <UploadCloud size={48} color="var(--primary-color)" className="mb-2" style={{margin: '0 auto'}} />
            <p><strong>Click to upload</strong> or drag and drop</p>
            <p style={{fontSize: '0.875rem', color: 'var(--text-secondary)'}}>PDF only (Max 10MB per file)</p>
            <input type="file" multiple accept=".pdf" ref={fileInputRef} style={{display: 'none'}} onChange={handleFileUpload} />
          </div>

          {batchStatus && (
            <div className="mt-4 p-4 border rounded" style={{borderColor: 'var(--border-color)'}}>
              <div className="flex justify-between mb-2">
                <strong>Processing Resumes</strong>
                <span>{batchStatus.processed + batchStatus.failed} / {batchStatus.total_resumes}</span>
              </div>
              <div style={{width: '100%', backgroundColor: 'var(--bg-color)', height: '8px', borderRadius: '4px'}}>
                <div style={{
              ...
              {batchStatus.status === 'Completed' && <p className="text-sm mt-2" style={{color: 'var(--success-color)'}}>Batch processing complete!</p>}
            </div>
          )}
        </div>
        ...
        <div className="card">
          ...
          <div className="mt-4">
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {job.mandatory_requirements?.split(',').map(s => s.trim() ? <span key={s} className="badge badge-warning">{s}</span> : null)}
            </div>
          </div>
        </div>
      </div>
            ...
            <tbody>
              {candidates.map(cand => (
                <tr key={cand.id}>
                  ...
                  <td><Link to={`/candidates/${cand.id}`} className="btn btn-secondary">Review</Link></td>
                </tr>
              ))}
